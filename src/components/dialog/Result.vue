...
watch(
    () => ({ loadStep: option.value.step.loadStep, subStep: option.value.step.subStep }),
    ({ loadStep, subStep }) => {
        let result = model.result
            .filter((res) => res.loadStep == loadStep)
            .at(-1)
...
const viewResultSecne = () => {
    ...
    if (!isMeshRsltExisted.value) {
        //所有节点freeze
        status.view.mesh.todo.freeze.node.prep = new Set(model.categorized.node.all)
        status.view.mesh.todo.freeze.node.rslt = new Set(model.categorized.node.free)
        //所有节点freeze
        status.view.mesh.todo.freeze.elem.prep = new Set(model.categorized.elem.free)
        //隐藏mesh text
        ...
        status.view.text.activated.node.rslt = new Set(model.categorized.node.free)
        status.view.text.activated.elem.rslt = new Set(model.categorized.elem.free)
...
<template>
    <Dialog :title="title" :left="10" :top="100" :width="250" :show="show">
        <el-form ref="form" :model="option" label-position="top">
            ...
            <el-form-item>
                ...
                <div class="grid-Wrapper1" v-show="option.nodeTag.show">
            ...
            <el-form-item>
                ...
                <div class="grid-Wrapper1" v-show="option.elemTag.show">
                    <div class="align-right">类别：</div>
                    <div>
            ...
            <el-form-item label="显示控制">
                <div class="grid-Wrapper2">
                    ...
                    <div style="flex: 1">
                        <el-checkbox v-model="option.legend.show"
                            >图例</el-checkbox
                        >
                        <el-button size="small">...</el-button>
                    </div>

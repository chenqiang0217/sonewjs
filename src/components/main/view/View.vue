...
const curFrame = computed(() => {
    return (
        status.task.view.animation.index *
            status.task.view.animation.frameRate +
        status.task.view.animation.iFrame
    )
})
...
<template>
    ...
    <div class="wrapper" v-loading="status.view.loading">
        ...
        <div class="animation" v-if="status.task.view.animation.show">
            <template v-for="(toolBar, i) in toolBars" :key="i">
                <el-tooltip
            ...
            <el-progress
                :percentage="
                    sumFrame == 0
                        ? 0
                        : Number.parseInt((curFrame * 100) / sumFrame)
                ">
                <el-button
                    text
                    v-if="showTime == true"
                    ...
                    >{{
                        (Math.floor(curFrame / 3600) < 10
                            ? '0' + Math.floor(curFrame / 3600)
                            : Math.floor(curFrame / 3600)) +
                        ':' +
                        (Math.floor(curFrame / 60) % 60 < 10
                            ? '0' + (Math.floor(curFrame / 60) % 60)
                            : Math.floor(curFrame / 60) % 60) +
                        '/' +
                        (Math.floor(sumFrame / 3600) < 10
                            ? '0' + Math.floor(sumFrame / 3600)
                            : Math.floor(sumFrame / 3600)) +
                        ':' +
                        (Math.floor(sumFrame / 60) % 60 < 10
                            ? '0' + (Math.floor(sumFrame / 60) % 60)
                            : Math.floor(sumFrame / 60) % 60)
                    }}</el-button
                >
